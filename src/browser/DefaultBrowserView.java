...
public class DefaultBrowserView implements BrowserView {
	static {
	...
	private Runnable swtDisplayAndBrowserCreateAndRun = new Runnable() {
		public void run() {
			DefaultBrowserView.this.swtDisplay = new Display();
	...
	public DefaultBrowserView(HttpHandler handler) {
		this(handler, true);
	}
	
	public DefaultBrowserView(HttpHandler handler, boolean runSwtLoop) {
		this(handler, lastPort++, runSwtLoop);
	}
	
	public DefaultBrowserView(HttpHandler handler, int port, final boolean runSwtLoop) {
		this.port = port;
